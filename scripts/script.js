...
browser.storage.local.get("logged", ({ logged }) => {
    if(logged) {
        ...
        // loadLists
        browser.storage.local.get("token", async ({ token }) => {
...
browser.storage.local.get("user", ({ user }) => {
  if(user) {
...
browser.storage.local.get("lists", ({ lists }) => {
  if(lists) {
    console.log(lists);
